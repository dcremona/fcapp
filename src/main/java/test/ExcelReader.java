...
public class ExcelReader {
    ...
    public static void main(String[] args) throws Exception, InvalidFormatException {
        ...
        // processSingleFileXls(SAMPLE_XLSX_FILE_PATH_INPUT7,
        // SAMPLE_XLSX_FILE_PATH_OUTPUT7);
        // processSingleFileXls(SAMPLE_XLSX_FILE_PATH_INPUT8,
        // SAMPLE_XLSX_FILE_PATH_OUTPUT8);
    ...
    public static void buildUnionFileOutputXls() throws Exception {
        ...
        String[] filesOutput = new String[] { SAMPLE_XLSX_FILE_PATH_OUTPUT1, SAMPLE_XLSX_FILE_PATH_OUTPUT2,
                SAMPLE_XLSX_FILE_PATH_OUTPUT3, SAMPLE_XLSX_FILE_PATH_OUTPUT4, SAMPLE_XLSX_FILE_PATH_OUTPUT5,
                SAMPLE_XLSX_FILE_PATH_OUTPUT6 };
    ...
    public static void processSingleFileXls(String input, String output) throws Exception {
        ...
        for (Row row : sheet) {
            ...
            if ("A".equals(a.toUpperCase().trim()) || "B".equals(a.toUpperCase().trim())
                    || "C".equals(a.toUpperCase().trim()) || "D".equals(a.toUpperCase().trim())) {
                // RISPOSTA
     ...
     *
     *
     * public static void test(String[] args) throws DatatypeConfigurationException
     * {
     *
     ...
     *
     * int conta = 1; Domanda domanda = null; for (int i = 1; i < csvRecords.size();
     * i++) { CSVRecord record = csvRecords.get(i);
     *
     * String a = record.get(0); String b = record.get(1); String c = record.get(2);
     ...
     * File.createTempFile("temp_" + fileName, ".csv"); // // Delete temp file when
     * program exits. // temp.deleteOnExit(); // // // Write to temp file // output
     * = new BufferedWriter(new FileWriter(temp)); // output.write(data); // if
     * (output != null) { // output.close(); // } // // // DELETE // File f = new
     * File(path_csv + fileName + ".csv"); // if (f.exists()) { // f.delete(); // }
     * // // // Destination directory // // File dir = new File(path_csv); // //
     * create the destination file object // File dest = new File(path_csv +
     * fileName + ".csv"); // // // Move file to new directory // boolean success =
     * temp.renameTo(dest); // if (!success) { // // File was not successfully moved
     * // } // // } catch (Exception e) { // // } finally { // if (output != null) {
     ...
     * sheet.createRow(++rowCount); // int columnCount = 0; // // for (Object field
     * : aBook) { // Cell cell = row.createCell(++columnCount); // if (field
