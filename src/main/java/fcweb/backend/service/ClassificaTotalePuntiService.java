...
public class ClassificaTotalePuntiService {
    ...
    @Autowired
    public ClassificaTotalePuntiService(ClassificaTotalePuntiRepository classificaTotalePuntiRepository) {
    ...
    public List<FcClassificaTotPt> findByFcCampionatoAndFcGiornataInfo(FcCampionato campionato,
        ...
        List<FcClassificaTotPt> l = classificaTotalePuntiRepository.findByFcCampionatoAndFcGiornataInfo(campionato,
                giornataInfo);
        return l;
    }
    ...
    public FcClassificaTotPt findByFcCampionatoAndFcAttoreAndFcGiornataInfo(FcCampionato campionato, FcAttore attore,
            FcGiornataInfo giornataInfo) {
        FcClassificaTotPt l = classificaTotalePuntiRepository.findByFcCampionatoAndFcAttoreAndFcGiornataInfo(campionato,
                attore, giornataInfo);
        return l;
    }
    ...
    public List<ClassificaBean> getModelClassifica(int curr_gg) {
        ...
        List<ClassificaBean> cList = jdbcTemplate.query(query, new RowMapper<ClassificaBean>() {
            @Override
            public ClassificaBean mapRow(ResultSet rs, int rownumber) throws SQLException {
    ...
    public List<ClassificaBean> getModelGrafico(String idAttoreA, String idAttoreB, FcCampionato campionato) {
        ...
        query += " (select  sum(u.punti) from fc_giornata_ris u where u.id_giornata <= t.id_giornata and u.id_attore = t.id_attore and u.id_giornata >="
                + start + "  and u.id_giornata<=" + endClas + " )  as SOMMA,  ";
        query += " (select  sum(v.tot_pt_rosa) from  fc_classifica_tot_pt v where v.id_giornata <= t.id_giornata and v.id_attore = t.id_attore and v.id_giornata >="
                + start + "  and v.id_giornata<=" + end + " )  as SOMMA2,  ";
        query += " (select  sum(v.pt_tvst) from  fc_classifica_tot_pt v where v.id_giornata <= t.id_giornata and v.id_attore = t.id_attore and v.id_giornata >="
                + start + "  and v.id_giornata<=" + end + " )  as SOMMA3  ";
    ...
    public List<ClassificaBean> getModelGraficoEm(String idAttoreA, String idAttoreB, FcCampionato campionato) {
