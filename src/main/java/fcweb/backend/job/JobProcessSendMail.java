...
public class JobProcessSendMail {
    ...
    public byte[] getJasperRisultati(FcCampionato campionato, FcGiornataInfo giornataInfo, Properties p,
            String pathImg) {
        byte[] b = null;
        try {
    ...
    private HashMap<String, Collection<RisultatoBean>> buildData(FcCampionato campionato, FcAttore attore,
            Double totGiornata, FcGiornataInfo giornataInfo, String pathImg, boolean fc) throws Exception {
        ...
        if (totGiornata != null) {
            totaleGiornata = formatter
                    .format(totGiornata.doubleValue() / Double.parseDouble("" + Costants.DIVISORE_100));
        }
