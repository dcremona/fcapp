...
public class EmJobProcessGiornata {
    ...
    @ResponseBody
    public void emaggiornamentoPFGiornataOLD(String fileName) throws IOException {
    ...
    @ResponseBody
    public void emaggiornamentoPFGiornata(Properties p, String fileName, String idGiornata) {
        ...
        try {
            ...
            for (int i = 1; i < csvRecords.size(); i++) {
                ...
                if (giocatore != null) {
                    ...
                    infoNewGiocatore += "\n<br>" + "NOT FOUND " + idGiocatore + " " + cognGiocatore + " " + ruolo + " "
                            + squadra;
                }
            }
    ...
    @ResponseBody
    public void emaggiornamentoPFGiornataNoExcel(Properties p, String fileName, String idGiornata) {
        ...
        try {
            ...
            for (CSVRecord record : csvRecords) {
                ...
                giocatore = this.giocatoreRepository
                        .findByCognGiocatoreStartingWithAndFcSquadraAndFcRuolo(cognGiocatore, fcSquadra, fcRuolo);
                // giocatore =
                ...
                if (giocatore != null) {
                    ...
                    if (count_sv == 1 || count_sv == 2) {
                        LOG.info("new_voto - count_sv " + count_sv + " - " + giocatore.getCognGiocatore() + " new_voto "
                                + new_voto + " G = " + G + " CS " + CS + " TS " + TS);
                        // infoVoti += "\n" + "new_voto - count_sv " + count_sv
                        // + " - " + giocatore.getCognGiocatore() + " new_voto "
                        // + new_voto + " G = " + G + " CS " + CS + " TS " + TS
    ...
    @ResponseBody
    public void emalgoritmo(Integer giornata, FcCampionato campionato) throws Exception {
        ...
        for (FcAttore attore : l) {
            ...
            String query = "DELETE FROM fc_classifica_tot_pt WHERE ID_CAMPIONATO=" + campionato.getIdCampionato()
                    + " AND ID_ATTORE=" + attore.getIdAttore() + " AND ID_GIORNATA=" + giornata + "";
            jdbcTemplate.update(query);

            query = "INSERT INTO fc_classifica_tot_pt (ID_CAMPIONATO,ID_ATTORE,ID_GIORNATA,TOT_PT) VALUES ("
                    + campionato.getIdCampionato() + "," + attore.getIdAttore() + "," + giornata + "," + totPunti + ")";
            jdbcTemplate.update(query);
        }
            ...
            jdbcTemplate.query(sql, new ResultSetExtractor<String>() {
                ...
                public String extractData(ResultSet rs) throws SQLException, DataAccessException {
                    ...
                    while (rs.next()) {
                        ...
                        String query = "DELETE FROM fc_classifica_tot_pt WHERE ID_CAMPIONATO="
                                + campionato.getIdCampionato() + " AND ID_ATTORE=" + attore.getIdAttore()
                                + " AND ID_GIORNATA=" + giornata + "";
                        jdbcTemplate.update(query);

                        query = "INSERT INTO fc_classifica_tot_pt (ID_CAMPIONATO,ID_ATTORE,ID_GIORNATA,TOT_PT) VALUES ("
                                + campionato.getIdCampionato() + "," + attore.getIdAttore() + "," + giornata + ","
                                + totPunti + ")";
                        jdbcTemplate.update(query);
    ...
    public void aggiornaFlagAttivoGiocatore(int giornata) throws Exception {
        ...
        jdbcTemplate.update(
                "update fc_giocatore set flag_attivo = 1 where id_squadra in (select id_squadra_casa from fc_calendario_tim where codice_giornata = "
                        + giornata + ")");
        jdbcTemplate.update(
                "update fc_giocatore set flag_attivo = 1 where id_squadra in (select id_squadra_fuori from fc_calendario_tim where codice_giornata = "
                        + giornata + ")");

        jdbcTemplate.update("update fc_statistiche set flag_attivo = 0");
        jdbcTemplate.update(
                "update fc_statistiche set flag_attivo = 1 where nome_squadra in (select squadra_casa from fc_calendario_tim where codice_giornata = "
                        + giornata + ")");
        jdbcTemplate.update(
                "update fc_statistiche set flag_attivo = 1 where nome_squadra in (select squadra_fuori from fc_calendario_tim where codice_giornata = "
                        + giornata + ")");
    ...
    @ResponseBody
    public void eminserisciUltimaFormazione(int idAttore, int giornata) throws Exception {
        ...
        ins += "SELECT " + giornata + "," + idAttore
                + ",ID_GIOCATORE,ID_STATO_GIOCATORE,ORDINAMENTO,0 from fc_giornata_dett where id_giornata=" + prev_gg
        ...
        ins2 += "select " + giornata + "," + idAttore
                + ",FLAG_INVIO,DATA_INVIO from fc_giornata_dett_info where id_giornata=" + prev_gg + " and id_attore="
                + idAttore;
    ...
    public HashMap<Object, Object> initDbGiocatori(String httpUrlImg, String imgPath, String fileName,
        ...
        try {
            ...
            for (int i = 1; i < csvRecords.size(); i++) {
                ...
                // squadra);
                giocatore = this.giocatoreRepository
                        .findByCognGiocatoreStartingWithAndFcSquadraAndFcRuolo(cognGiocatore, squadra, ruolo);
                if (giocatore == null) {
                    // lastIdGiocatore++;
                    giocatore = new FcGiocatore();
    ...
    public HashMap<Object, Object> initDbGiocatoriExcel(InputStream is) throws Exception {
            ...
            for (Row row : sheet) {
                ...
                if (StringUtils.isNotEmpty(idGiocatore)) {
                    ...
                    for (FcGiocatore g : lgiocatore) {
                        ...
                        if (!g.getFcSquadra().getNomeSquadra().equals(nomeSquadra)) {
                            LOG.info("ATTENZIONE SQUADRA DIFFERENTE ");
                            LOG.info("" + idGiocatore + ";" + gameName + ";" + idRuolo + ";" + nomeSquadra + ";"
                    ...
                    if (giocatore == null) {
                        ...
                        System.out.println("" + idGiocatore + ";" + gameName + ";" + idRuolo + ";" + nomeSquadra + ";"
                                + quotazioneAttuale + ";" + quotazioneAttuale);
