...
@RolesAllowed("USER")
public class EmDownloadView extends VerticalLayout implements ComponentEventListener<ClickEvent<Button>> {
    ...
    private void setRoseA(VerticalLayout layout) {
        ...
        // gridRosea.addThemeVariants(GridVariant.LUMO_NO_BORDER,
        ...
        // StreamResource("Nazionali.csv",Exporter.exportAsCSV(gridRosea)),"Download As
        // CSV");
    ...
    private Column<FcExpRosea> getColumnR(Grid<FcExpRosea> grid, int i) {
        ...
        Column<FcExpRosea> rxColumn = grid.addColumn(new ComponentRenderer<>(f -> {
            ...
            if (ruolo != null && ("P".equals(ruolo) || "D".equals(ruolo) || "C".equals(ruolo) || "A".equals(ruolo))) {
                Image img = Utils.buildImage(ruolo.toLowerCase() + ".png",
    ...
    // StreamResource("StatisticheVoti.pdf",() -> {
    // byte[] b = null;
    // try {
    ...
    // Resource resource =
    // resourceLoader.getResource("classpath:reports/statisticheVoti.jasper");
    ...
    // Image img = buildImage(Costants.CLASSPATH_IMAGES,
    // g.getIdRuolo().toLowerCase() + ".png");
    // cellLayout.add(img);
    // }
    // return cellLayout;
    // }));
    ...
    //
    // Column<FcStatistiche> nomeSquadraColumn = grid.addColumn(new
    // ComponentRenderer<>(s -> {
    //
    // HorizontalLayout cellLayout = new HorizontalLayout();
    // cellLayout.setMargin(false);
    ...
    // Resource r = resourceLoader.getResource("classpath:" + "/img/nazioni/" +
    // s.getNomeSquadra() + ".png");
    ...
    //
    // Column<FcStatistiche> mediaVotoColumn = grid.addColumn(new
    // ComponentRenderer<>(s -> {
    //
    // HorizontalLayout cellLayout = new HorizontalLayout();
    // cellLayout.setMargin(false);
    ...
    //
    // Column<FcStatistiche> fantaMediaColumn = grid.addColumn(new
    // ComponentRenderer<>(s -> {
    // DecimalFormat myFormatter = new DecimalFormat("#0.00");
    // Double d = Double.valueOf(0);
