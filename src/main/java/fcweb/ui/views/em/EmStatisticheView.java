...
@RolesAllowed("USER")
public class EmStatisticheView extends VerticalLayout implements ComponentEventListener<ClickEvent<Button>> {
    ...
    public Component buildGrafico(FcCampionato campionato) {
        ...
        List<ClassificaBean> items = classificaTotalePuntiController.getModelGraficoEm(idAttoreA, idAttoreB,
                campionato);
    ...
    private void setStatisticheA(VerticalLayout layout, FcCampionato campionato, Properties p, FcAttore att) {
        ...
        Column<FcStatistiche> ruoloColumn = grid.addColumn(new ComponentRenderer<>(s -> {
            ...
            if (s != null && s.getIdRuolo() != null) {
                Image img = Utils.buildImage(s.getIdRuolo().toLowerCase() + ".png",
                        resourceLoader.getResource(Costants.CLASSPATH_IMAGES + s.getIdRuolo().toLowerCase() + ".png"));
                cellLayout.add(img);
            }
            return cellLayout;
        }));
