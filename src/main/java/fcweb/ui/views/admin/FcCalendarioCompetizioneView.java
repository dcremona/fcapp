...
public class FcCalendarioCompetizioneView extends VerticalLayout
	...
	private void initLayout() {
		...
		Column<FcCalendarioCompetizione> dataColumn = crud.getGrid().addColumn(new LocalDateTimeRenderer<>(FcCalendarioCompetizione::getData,() -> DateTimeFormatter.ofPattern(Costants.DATA_FORMATTED)));
		...
		try {
			...
			if (event.getSource() == initDb) {
				...
				if ("1".equals(campionato.getType())) {
					...
					// String httpUrl = urlFanta + calendario +
					// ".asp?GiornataA=" + giornata +
					// "&Tipolink=0";
					// LOG.info("httpUrl " + httpUrl);
