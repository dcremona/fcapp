...
public class FcGiocatoreView extends VerticalLayout {
    ...
    private void initLayout() {
        ...
        formFactory.setVisibleProperties(CrudOperation.READ, "idGiocatore", "cognGiocatore", "quotazione", "nomeImg",
                "fcSquadra", "fcRuolo", "flagAttivo", "quotazione");
        formFactory.setVisibleProperties(CrudOperation.ADD, "idGiocatore", "cognGiocatore", "nomeImg", "fcSquadra",
                "fcRuolo", "flagAttivo", "quotazione");
        formFactory.setVisibleProperties(CrudOperation.UPDATE, "cognGiocatore", "quotazione", "nomeImg", "fcSquadra",
                "fcRuolo", "flagAttivo", "quotazione");
        ...
        crud.getGrid().addColumn(new TextRenderer<>(g -> g == null ? "" : g.getFcRuolo().getIdRuolo()))
                .setHeader(Costants.RUOLO);

        Column<FcGiocatore> giocatoreColumn = crud.getGrid()
                .addColumn(new TextRenderer<>(g -> g == null ? "" : "" + g.getCognGiocatore()))
        ...
        Column<FcGiocatore> squadraColumn = crud.getGrid()
                .addColumn(new TextRenderer<>(g -> g == null ? "" : g.getFcSquadra().getNomeSquadra()))
                .setHeader(Costants.SQUADRA);
        squadraColumn.setSortable(false);
        squadraColumn.setAutoWidth(true);

        crud.getGrid().addColumn(new TextRenderer<>(g -> g == null ? "" : "" + g.getQuotazione()))
                .setHeader("Quotazione");

        crud.getGrid().setColumnReorderingAllowed(true);

        formFactory.setFieldProvider("fcSquadra", new ComboBoxProvider<>("fcSquadra", squadraController.findAll(),
                new TextRenderer<>(FcSquadra::getNomeSquadra), FcSquadra::getNomeSquadra));
        formFactory.setFieldProvider("fcRuolo", new ComboBoxProvider<>("fcRuolo", ruoloController.findAll(),
                new TextRenderer<>(FcRuolo::getDescRuolo), FcRuolo::getDescRuolo));
        ...
        crud.setFindAllOperation(() -> giocatoreController
                .findByFcRuoloAndFcSquadraOrderByQuotazioneDesc(ruoloFilter.getValue(), squadraFilter.getValue()));
