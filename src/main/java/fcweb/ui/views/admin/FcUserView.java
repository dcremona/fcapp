...
public class FcUserView extends VerticalLayout {
    ...
    private void initLayout() {
        ...
        formFactory.setVisibleProperties(CrudOperation.READ, "id", "username", "hashedPassword", "name", "roles",
                "profilePicture", "idAttore", "descAttore", "cognome", "nome", "cellulare", "email", "notifiche",
                "active");
        formFactory.setVisibleProperties(CrudOperation.ADD, "id", "username", "hashedPassword", "name", "roles",
                "profilePicture", "idAttore", "descAttore", "cognome", "nome", "cellulare", "email", "notifiche",
                "active");
        formFactory.setVisibleProperties(CrudOperation.UPDATE, "username", "hashedPassword", "name", "roles",
                "profilePicture", "idAttore", "descAttore", "cognome", "nome", "cellulare", "email", "notifiche",
                "active");
         ...
         * 
         * FileBuffer fileBuffer = new FileBuffer(); Upload singleFileUpload = new
         * Upload(fileBuffer); singleFileUpload.setDropAllowed(true);
         * singleFileUpload.addSucceededListener(event -> { try {
         * 
         * // Get information about the uploaded file InputStream fileData =
         * fileBuffer.getInputStream(); // String fileName = event.getFileName(); //
         * long contentLength = event.getContentLength(); // String mimeType =
         * event.getMIMEType();
         * 
         * ByteArrayOutputStream buffer = new ByteArrayOutputStream(); int nRead; byte[]
         * data = new byte[4]; while ((nRead = fileData.read(data, 0, data.length)) !=
         * -1) { buffer.write(data, 0, nRead); } buffer.flush(); byte[] targetArray =
         * buffer.toByteArray(); u.setProfilePicture(targetArray);
         * attoreService.update(u);
         * CustomMessageDialog.showMessageInfo(CustomMessageDialog.MSG_OK); } catch
         * (Exception e) {
